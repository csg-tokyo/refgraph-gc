...
#
# The instances of Planner, Plan, Variable, and som.Vector are allocated
...
class Planner
  ...
  def remove_propagate_from(out_v)
    ...
    out_v.set_walk_strength(ABSOLUTE_WEAKEST)
    out_v.stay = true
...
class Strength
  ...
  def self.sym_to_id(arr, sym)
    arr[sym.custom_hash] = sym
  end

  def self.create_strength_sym_table
    table = []
    sym_to_id(table, SYM_ABSOLUTE_STRONGEST)
    sym_to_id(table, SYM_REQUIRED)
    sym_to_id(table, SYM_STRONG_PREFERRED)
    sym_to_id(table, SYM_PREFERRED)
    sym_to_id(table, SYM_STRONG_DEFAULT)
    sym_to_id(table, SYM_DEFAULT)
    sym_to_id(table, SYM_WEAK_DEFAULT)
    sym_to_id(table, SYM_ABSOLUTE_WEAKEST)
    table
  end

  STRENGHT_SYM_TABLE = create_strength_sym_table

  def self.symbol(id)
    STRENGHT_SYM_TABLE[id]
  end
end
...
class AbstractConstraint
  ...
  def destroyConstraint(planner)
    destroy_constraint(planner)
  end
  ...
  def self.sort(constraints)
    constraints.sort { |c1, c2| c1.strength.stronger(c2.strength) }
    constraints
  end
...
class BinaryConstraint < AbstractConstraint
  ...
  def recalculate
    ...
    outx.set_walk_strength(@strength.weakest(ihn.walk_strength))
    outx.stay = ihn.stay
...
class UnaryConstraint < AbstractConstraint
  ...
  def recalculate
    @output.set_walk_strength(@strength)
...
class Variable
  ...
  def set_determined_by(d)
    @determined_by = d
  end

  def set_walk_strength(w)
    @walk_strength = w
  end
...
class Factory
  def editConstraint(v, strength_id, planner)
    EditConstraint.new(v, Strength.symbol(strength_id), planner)
  end

  def equalityConstraint(var1, var2, strength_id, planner)
    EqualityConstraint.new(var1, var2, Strength.symbol(strength_id), planner)
  end

  def stayConstraint(v, strength_id, planner)
    StayConstraint.new(v, Strength.symbol(strength_id), planner)
  end

  def scaleConstraint(src, scale, offset, dst, strength_id, planner)
    ScaleConstraint.new(src, scale, offset, dst, Strength.symbol(strength_id), planner)
  end

  def sort(constraints)
    AbstractConstraint.sort(constraints)
  end
...
def benchmark()
  Jscall.config module_names: [['DeltaBlue', './', 'deltablue.mjs']]
  p Jscall.DeltaBlue.benchmark(250)
end
...
def benchmark_ruby(n)
  n.times do |i|
    t0 = Refgraph.clock
    unless DeltaBlue.new.inner_benchmark_loop(25000)
      raise "#{i}-th iteration failed"
    end
    puts Refgraph.clock(t0, "#{i}-th ")
  end
  true
end
...
def run_benchmark()
  ...
  Refgraph.config module_names: [['DeltaBlue', './', 'deltablue.mjs']]
  # Refgraph.use_bloomfilter(true)
  GC::Profiler.enable
  #Refgraph.gc_off
  #Refgraph.run_ruby_gc
  10.times do
    t00 = Refgraph.clock
    Jscall.DeltaBlue.benchmark(250)
    #Refgraph.gc
