...
{
    return GET_HEAP_PAGE(p)->extension;
}
...
void refgraph_for_each_heap_page_extension(void* callback(void*, size_t))
{
	...
	struct heap_page *page = heap_pages_sorted[i];
        /*
         * HEAP_PAGE_BITMAP_LIMIT is the size of a bits_t array used
         * as a bitmap.
         */
        page->extension = callback(page->extension, HEAP_PAGE_BITMAP_LIMIT);
    }
}

size_t refgraph_num_in_page(VALUE p)
{
    return NUM_IN_PAGE(p);
}

uintptr_t refgraph_marked_in_bitmap(uintptr_t* bits, VALUE p)
{
    return MARKED_IN_BITMAP(bits, p);
}

void refgraph_mark_in_bitmap(uintptr_t* bits, VALUE p)
{
    MARK_IN_BITMAP(bits, p);
}
